...
import styles from "./LuminousFluxScene.module.scss"; // Archivo actualizado
...
const DynamicFluxMaterial = shaderMaterial(
  {
    uTime: 0,
    uFrequency: 4.5,
    uAmplitude: 0.35,
...
extend({ DynamicFluxMaterial });

// Actualizamos la declaración de tipos para el nuevo material
declare global {
  namespace JSX {
    interface IntrinsicElements {
      dynamicFluxMaterial: ReactThreeFiber.Object3DNode<
...
function FloatingImage() {
  ...
  useEffect(() => {
    if (ref.current) {
      // Animación inicial con valores más pronunciados
      ...
      // Aumentamos los valores para que los efectos sean más visibles
      gsap.to(ref.current, {
        uFrequency: 4.5,
        uAmplitude: 0.35,
        duration: 3,
  ...
  const geometry = useMemo(() => new THREE.PlaneGeometry(3, 2, 48, 48), []);
...
export default function LuminousFluxScene() {
