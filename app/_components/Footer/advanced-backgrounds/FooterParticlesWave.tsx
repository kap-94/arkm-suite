...
class Particle {
  ...
  constructor({
    ...
    this.waveSpeed = randomRange(0.01, 0.03);
    this.waveRadius = randomRange(5, 30);
  }

  update(cursor: Vector) {
    // Onda/pulso alrededor de (ix, iy)
    this.waveAngle += this.waveSpeed;
    const waveOffset = Math.sin(this.waveAngle) * this.waveRadius;
    // "distorsión" en X e Y (ej: horizontal wave en x, vertical wave en y, etc.)
    const waveX = this.ix + waveOffset;
    const waveY = this.iy + Math.cos(this.waveAngle) * this.waveRadius * 0.5;

    // Atracción hacia la posición de la onda
    let dx = waveX - this.x;
    let dy = waveY - this.y;
...
export default function FooterParticlesWave({
  imageA,
  imageB,
  ...
  // getImageData, createParticles, createTemporaryParticles
  const getImageData = (
