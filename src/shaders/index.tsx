...
export const fragmentShaderSmoothHologram = `
  ...
  void main() {
    ...
    if (texColor.a > 0.0) {
      ...
      float t2 = sin(time * 1.2 + uv.x * 3.0) * 0.5 + 0.5;
...
//     float slices = 20.0;
//     float sliceProgress = fract(time * 0.2);
...
//     float slices = 20.0;
//     float sliceProgress = fract(delayedTime * 0.2);
//     float sliceY = floor(uv.y * slices) / slices;
...
//         float xProgress = fract(delayedTime * 0.2);
//         vec2 distortedUv = uv + vec2(sliceOffset, 0.0);
...
// GRUPO 2: EFECTOS TECH Y DIGITALES
...
//             float pulseBrightness = 1.0 + pulse * 0.2;
...
//             vec3 baseColor = vec3(0.95);
//             float energyGlow = (emWave * 0.5 + 0.5) * fieldStrength * 0.2;
...
void main() {
    ...
    if (time < slicingStart) {
        ...
        if (max(mainColor.a, chargeColor.a) > 0.0) {
            vec3 baseColor = vec3(0.95);
            float energyGlow = (emWave * 0.5 + 0.5) * fieldStrength * 0.2 * (1.0 - transitionProgress);
            vec3 finalColor = baseColor + vec3(energyGlow * 0.5, energyGlow, energyGlow * 1.5);
            gl_FragColor = vec4(finalColor, mainColor.a);
            return;
        }
...
void main() {
    ...
    if (time < slicingStart) {
        ...
        float effectStrength = (1.0 - transitionProgress) * (1.0 + pulse * 0.2);
        ...
        if (max(mainColor.a, resonanceColor.a) > 0.0) {
            vec3 baseColor = vec3(0.95);
            float digitalGlow = (resonance * 0.5 + 0.5) * (1.0 - dist) * effectStrength * 0.2;
...
void main() {
    ...
    if (time < slicingStart) {
        ...
        if (max(mainColor.a, energyColor.a) > 0.0) {
            vec3 baseColor = vec3(0.95);
            float energyGlow = energySpiral * 0.2 * vortexPull * effectStrength;
...
void main() {
    ...
    if (time < slicingStart) {
        ...
        if (max(mainColor.a, glowColor.a) > 0.0) {
            vec3 baseColor = vec3(0.95);
            float glow = threadGlow * effectStrength * 0.2;
...
void main() {
    ...
    if (time < slicingStart) {
        ...
        for(float i = 1.0; i <= 3.0; i++) {
            // Ondas circulares
            float circleWave = sin(dist * 20.0 * i - time * (2.0 + i * 0.5));
            // Ondas horizontales
...
void main() {
    ...
    if (time < initialEffectDuration) {
        ...
        if (max(mainColor.a, glowColor.a) > 0.0) {
            vec3 baseColor = vec3(0.95);
            float digitalGlow = matrix * 0.2 * (1.0 - rainDrop);
...
void main() {
    ...
    if (time < slicingStart) {
        ...
        if (texColor.a > 0.0) {
            vec3 baseColor = vec3(0.95);
            float auroraGlow = (auroraWave * 0.5 + 0.5) * (1.0 - transitionProgress) * 0.2;
...
void main() {
    ...
    if (time < initialEffectDuration) {
        ...
        if (max(max(redChannel.a, greenChannel.a), blueChannel.a) > 0.0) {
            ...
            vec3 holoColor = mix(
                vec3(0.95),
                vec3(redChannel.r * 1.2, greenChannel.g, blueChannel.b * 1.1),
                holoPulse * 0.3
            );
    ...
    // Transición (2-4s): Descomposición espectral
    else if (time < slicingStart) {
        float transitionProgress = smoothstep(initialEffectDuration, initialEffectDuration + transitionDuration, time);
...
void main() {
    ...
    if (time < initialEffectDuration) {
        ...
        if (max(mainChannel.a, quantumShift.a) > 0.0) {
            ...
            vec3 finalColor = mix(baseColor, vec3(0.95, 0.98, 1.0), quantumFlux * 0.2);
            gl_FragColor = vec4(finalColor, mainChannel.a * 0.95);
            return;
        }
    }
    // Transición (2-4s): Tunelización cuántica
    else if (time < slicingStart) {
        float transitionProgress = smoothstep(initialEffectDuration, initialEffectDuration + transitionDuration, time);
...
void main() {
    ...
    if (time < initialEffectDuration) {
        ...
        if (max(texColor.a, dataShift.a) > 0.0) {
            ...
            vec3 finalColor = mix(baseColor, vec3(1.0), dataStream * dataPulse * 0.2);
            gl_FragColor = vec4(finalColor, texColor.a * 0.95);
            return;
        }
    }
    // Transición (2-4s): Compilación de datos
    else if (time < slicingStart) {
        float transitionProgress = smoothstep(initialEffectDuration, initialEffectDuration + transitionDuration, time);
...
void main() {
    ...
    if (time < initialEffectDuration) {
        ...
        if (max(mainChannel.a, energyShift.a) > 0.0) {
            ...
            vec3 finalColor = mix(baseColor, vec3(1.0), energyGlow * pulse * 0.2);
            gl_FragColor = vec4(finalColor, mainChannel.a * 0.95);
            return;
        }
    }
    // Transición (2-4s): Campo de energía
    else if (time < slicingStart) {
        float transitionProgress = smoothstep(initialEffectDuration, initialEffectDuration + transitionDuration, time);
...
void main() {
    ...
    // Transición (2-4s): Disolución digital
    else if (time < slicingStart) {
        float transitionProgress = smoothstep(initialEffectDuration, initialEffectDuration + transitionDuration, time);
...
export const fragmentShaderTechGrid = `
  ...
  void main() {
    ...
    if (texColor.a > 0.0) {
      ...
      finalColor += indigoLight * pow(1.0 - depth, 4.0) * 0.2;
...
export const fragmentShaderQuantumField = `
  ...
  void main() {
    ...
    if (texColor.a > 0.0) {
      ...
      float q2 = quantum(uv * 2.0 - time * 0.2);
...
export const fragmentShaderIridescentCrystal = `
  ...
  void main() {
    ...
    if (texColor.a > 0.0) {
      ...
      vec3 color2 = iridescent(angle2 * 0.2 - time * 0.05);
...
export const fragmentShaderMatrixRain = `
  ...
  void main() {
    ...
    if (texColor.a > 0.0) {
      ...
      float brightness = (1.0 - dropSpeed) * 1.2;
...
export const fragmentShaderMysticRunes = `
  ...
  void main() {
    ...
    if (texColor.a > 0.0) {
      ...
      float r2 = rune((uv + 0.5) * 1.2 - time * 0.05, 6.0);
...
void main() {
    ...
    float slices = 12.0;
    float sliceY = floor(uv.y * slices) / slices;
    float sliceProgress = smoothstep(0.0, 1.0, delayedTime - sliceY * 0.2);
